...
const wb_service_1 = require("./wb.service");
describe("WeightBalanceService", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        service = new wb_service_1.WeightBalanceService(mockAircraftService);
    });
    describe("calculateCG", () => {
        ...
        it("throws when any load weight is negative", async () => {
            ...
            const loads = { [stationId]: -50 };
            // Harekete Geç (Act)
            const act = service.calculateCG(aircraftId, loads);
            // Doğrula (Assert)
            await expect(act).rejects.toThrow("Weight at station 'station-1' cannot be negative");
            await expect(act).rejects.toThrow(Error);
        });
    });
});
//# sourceMappingURL=wb.service.test.js.map